...
def save_data(data):
    ...
    try:
        ...
        with open(BASE_DIR.joinpath(FILE_STORAGE), 'r', encoding='utf-8') as fd:
            try:
                data = json.load(fd)
            except JSONDecodeError:
                data = dict()  # аби обійти помилку читання, якщо файл ще пустий або містить некоректний JSON
        data[str(datetime.datetime.now())] = payload
        with open(BASE_DIR.joinpath(FILE_STORAGE), 'w', encoding='utf-8') as fd:
            json.dump(data, fd)
